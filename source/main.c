...
int main(void)
{
    ...
    //nampt
    uint8_t val_trans[] = {01,02,03,04,12};
    flash_erase_sector(USER_SPI1_MASTER,Start_Add_FLASH_EX);
    flash_write_buffer(USER_SPI1_MASTER, val_trans, Start_Add_FLASH_EX,4);
    ...
    flash_read_buffer(USER_SPI1_MASTER,&system_var.u8NewFirmFlag3, Start_Add_FLASH_EX,1);
...
*/
    /* Prepare user buffer. */
    uint8_t val_tran[]={0x01,0x02,0x03,0x04};
    flash_erase_sector(USER_SPI1_MASTER,ADD_FLEX);  
    flash_write_buffer(USER_SPI1_MASTER, sample_file, ADD_FLEX,1024);
    
    uint32_t positionFirm = ADD_FLEX;
    uint32_t des_position = destAdrss;
    uint32_t current_pos = 0 ;
    uint32_t j= 0;
    if(FLASH_Erase(&s_flashDriver, destAdrss, pflashSectorSize, kFLASH_ApiEraseKey)== kStatus_FLASH_Success)
    {
      do{
        flash_read_buffer(USER_SPI1_MASTER,g_buffer_flashEX,positionFirm,1024);
        for(i=0;i<512;i++)
        {
          s_buffer[i]=g_buffer_flashEX[j]|g_buffer_flashEX[1+j]<<8|g_buffer_flashEX[j+2]<<16|g_buffer_flashEX[j+3]<<24;
          j=j+4;
         }
      if(FLASH_Program(&s_flashDriver, destAdrss, s_buffer, sizeof(s_buffer)) == kStatus_FLASH_Success)
      {
        des_position += 1024;
      }
      current_pos +=1024;
    ...
    {
      s_buffer_rbc[k] = *(volatile uint32_t *)(destAdrss+k*4);
    }
